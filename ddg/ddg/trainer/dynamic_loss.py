...
__all__ = ['DDG_Trainer_Loss']


class DDG_Trainer_Loss(Trainer):
    def __init__(self):
        # Trainer.__init__(self)
        super(DDG_Trainer_Loss, self).__init__()
    ...
    def build_criterion(self):
        ...
        self.cosine_pairwise_loss = cosine_pairwise_loss()
        self.criterion = [self.ce, self.cosine_pairwise_loss]
    ...
    def model_forward_backward(self, batch):
        ...
        loss = torch.tensor(self.criterion[0](output, target), requires_grad=True) + torch.tensor(self.criterion[1](output,target), requires_grad=True)
